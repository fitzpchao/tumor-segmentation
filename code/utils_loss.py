...
import numpy as np
from losses import lovasz_hinge as lh
...
class Trainer(object):
    ...
    def __init__(self, model, optimizer, loss_f, save_dir=None, save_freq=1):
        ...
        #self.model.load_state_dict(torch.load("E:/pc/cd_pixel/checkpoints/sexp6_7_DA_2\ model_237.pkl")['weight'])
    ...
    def _iteration(self, data_loader, ep ,is_train=True):
        ...
        for img,mask in tqdm(data_loader):
            #print(data.shape)
            #print(target.shape)
            img,target=img.cuda(),mask.cuda()
            output= self.model(img)
            loss1 = 0.5*self.loss_f(F.sigmoid(output),target)
            loss2 = 0.5 * self.a_loss_f(output.squeeze(1),target.squeeze(1))
            loss = loss1 + loss2
